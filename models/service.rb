        ...
        request.callback do
          ...
          if request.response_header.status == 200
            ...
            if service_responses.empty? && queries.length > 0
              ...
              @logger.info "Service #{self.class} took #{(Time.now - start)}"
              self.succeed(service_responses)
            end
          else
            self.fail("Service #{self.class} failed with status #{request.response_header.status} for url: #{@configuration['url']}, query: #{query}")
          end
        end
